...
#define SPLIT_BLOCK_SIZE 100000
#define MAP_BLOCK_SIZE 400000
...
*/
    ...
    void to_binary() {
        ...
        if (lineitem_file == NULL) {
            printf("lineitem.tbl open error\n");
            exit(0);
        }
...
*/	
    ...
    void word_count_print_function(FILE* reduce_file) {
    ...
    void word_count_print() {
        ...
        word_count_print_function(reduce_file1);
        word_count_print_function(reduce_file2);
        word_count_print_function(reduce_file3);
...
public:
    ...
    void run() {
        clock_t map_1, map_2, reduce_1, reduce_2;			// 시간을 체크하기 위한 변수
	...
	sleep(3);
        ...
        printf("map 실행 시간 : %f 초\n", ((double)(map_2 - map_1)) / CLOCKS_PER_SEC);
        printf("reduce 실행 시간 : %f 초\n", ((double)(reduce_2 - reduce_1)) / CLOCKS_PER_SEC);
        printf("메모리 사용량: %ld KB\n", r_usage.ru_maxrss);
    }
};
